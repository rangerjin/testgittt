...
<body>
	...
	<div id="main">
		...
		<div class="w3-container">
			...
			<div class="loginDiv">
				<form method="POST" action="/ceo/updateMenu.do" method="post"
					enctype="multipart/form-data">
					<!-- 메뉴이름 -->
					<div class="form-group">
						<div class="col-sm-6">
							<label for="menu_name">메뉴 이름</label> <input type="text"
								class="form-control" id="menu_name" name="menu_name" value="<%=mDTO.getMenu_name() %>"
					...
					<div class="row">
						<div class="col-sm-6">
							<label for="menu_cost">가격</label> <input type="text"
								id="menu_cost" name="menu_cost" placeholder="가격" value="<%=mDTO.getMenu_cost() %>"
						...
						<div class="form-group">
							...
							<div class="col-sm-4">
								<div class="select_img" id="select_img">
									<img src="/resources/menuImg/<%=mDTO.getMenu_img()%>" />
								</div>
							</div>
						</div>
					</div>
						...
						<div class="form-group">
							<div class="col-sm-6">
								...
								<textarea rows="5" cols="100" style="resize: none;"
									id="menu_exp" name="menu_exp"><%=mDTO.getMenu_exp() %></textarea>
							</div>
						</div>
					</div>
					...
					<!-- 가게 번호 -->
					<input type="hidden" name="menu_str_no" id="menu_str_no" value="<%=mDTO.getMenu_str_no() %>" />
					<input type="hidden" name="menu_no" id="menu_no" value="<%=mDTO.getMenu_no() %>" />
					<input type="hidden" name="menuNowPage" value="${param.menuNowPage }" />
					<!-- 작성자 -->
					<input type="hidden" name="user_id" id="user_id" value="<%=user_id %>" />
					<input type="hidden" name="menu_img" id="menu_img" value="<%=mDTO.getMenu_img()%>" />
				</form>
				<br> <br>
			</div>
		</div>
